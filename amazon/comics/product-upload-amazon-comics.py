...
def upload_product_to_woocommerce(product, existing_titles):
    ...
    try:
        modified_price = str(round((float(price) / 19) * 2, 2))  # 價格修改為除以4.2再乘以2
    except ValueError:
    ...
    data = {
        ...
        "categories": [{"id": 34}]  # 使用分類 ID 而不是名稱
    }
...
if __name__ == '__main__':
    # 設定 CSV 文件夾的路徑
    csv_folder_path = '/home/ubuntu/books/amazon/comics'  # 替換為包含所有 CSV 文件的文件夾路徑
